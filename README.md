&emsp;&emsp;我们决定使用面向对象的思想来开发这个游戏。也许“在汇编语言里面向对象”听起来有些疯狂，但我们讨论后认为这完全可行：面向对象是一种设计思想。C++等高级语言只是提供了便于沿用这个思想开发的工具罢了————这些高级语言最后还是要被编译成机器码，可以被反编译为汇编的，不是吗？换句话说，我们的工作是用高级语言的面向对象思想开发，然后跳过编译器的部分，直接手动写成汇编的实现。
...
## 零散的数值与一些公式
### 关于移动与帧的关系
&emsp;&emsp;初步构想的是每秒移动100个像素。每帧至少1/26s，这里取1/50s，也就是0.02s。那么每秒就是50帧，每帧移动像素100/50 = 2个。
### 关于移动速度
&emsp;&emsp;一个物体的移动速度数值为x，表示该物体每10帧移动的像素。按上文每帧移动2个像素计算的话该数值为20
